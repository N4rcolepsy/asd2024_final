...
class double_buffered_scratchpad:
    def __init__(self, double_buffer=True):
        ...
        if self.double_buffer:
            ...
            self.ifmap_L3_buf = rdbuf()
            self.filter_L3_buf = rdbuf()
        else:
    ...
    def set_params(self,
        ...
        if self.estimate_bandwidth_mode:
            ...
            if self.double_buffer:
                ...
                self.ifmap_L2_buf.set_params(backing_buf_obj=self.ifmap_L3_buf,
                                             total_size_bytes=ifmap_buf_size_bytes * 2,
                                             word_size=word_size,
                                             active_buf_frac=rd_buf_active_frac,
                                             backing_buf_default_bw=ifmap_backing_buf_bw)

                self.ifmap_L3_buf.set_params(backing_buf_obj=self.ifmap_port,
                ...
                self.filter_L2_buf.set_params(backing_buf_obj=self.filter_L3_buf,
                                              total_size_bytes=filter_buf_size_bytes * 2,
                                              word_size=word_size,
                                              active_buf_frac=rd_buf_active_frac,
                                              backing_buf_default_bw=filter_backing_buf_bw)

                self.filter_L3_buf.set_params(backing_buf_obj=self.filter_port,
    ...
    def get_ifmap_dram_details(self):
        ...
        if self.double_buffer:
            self.ifmap_dram_reads = self.ifmap_L3_buf.get_num_accesses()
            self.ifmap_dram_start_cycle, self.ifmap_dram_stop_cycle = self.ifmap_L3_buf.get_external_access_start_stop_cycles()
        else:
            self.ifmap_dram_reads = self.ifmap_buf.get_num_accesses()
            self.ifmap_dram_start_cycle, self.ifmap_dram_stop_cycle = self.ifmap_buf.get_external_access_start_stop_cycles()
    ...
    def get_filter_dram_details(self):
        ...
        if self.double_buffer:
            self.filter_dram_reads = self.filter_L3_buf.get_num_accesses()
            self.filter_dram_start_cycle, self.filter_dram_stop_cycle = self.filter_L3_buf.get_external_access_start_stop_cycles()
        else:
            self.filter_dram_reads = self.filter_buf.get_num_accesses()
            self.filter_dram_start_cycle, self.filter_dram_stop_cycle = self.filter_buf.get_external_access_start_stop_cycles()
    ...
    def get_ofmap_dram_details(self):
        ...
        self.ofmap_dram_start_cycle, self.ofmap_dram_stop_cycle = self.ofmap_buf.get_external_access_start_stop_cycles()
    ...
    def get_ifmap_dram_trace_matrix(self):
        ...
        if self.double_buffer:
            return self.ifmap_L3_buf.get_trace_matrix()
        else:
    ...
    def get_filter_dram_trace_matrix(self):
        ...
        if self.double_buffer:
            return self.filter_L3_buf.get_trace_matrix()
        else:
    ...
    def get_dram_trace_matrices(self):
        ...
        if self.double_buffer:
            dram_ifmap_trace = self.ifmap_L3_buf.get_trace_matrix()
            dram_filter_trace = self.filter_L3_buf.get_trace_matrix()
    ...
    def print_ifmap_dram_trace(self, filename):
        ...
        if self.double_buffer:
            self.ifmap_L3_buf.print_trace(filename)
        else:
            self.ifmap_buf.print_trace(filename)

    def print_filter_dram_trace(self, filename):
        if self.double_buffer:
            self.filter_L3_buf.print_trace(filename)
        else:
