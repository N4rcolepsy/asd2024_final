...
if a==1:
    logging.basicConfig(
        filename="activation_compress_ratios_input.log",  # 로그 파일 이름
...
def main():
    ...
    for name, layer in model.named_modules():
        ...
        if a == 1 :
            if isinstance(layer, (nn.Linear, nn.Conv2d)):
                layer.register_forward_hook(get_activation_i(name))
        if a == 0: 
            if isinstance(layer, (nn.Linear, nn.Conv2d)):
    ...
    if a == 1:
        for name, activation in activation_values.items():
            ...
            # zero ratio 계산
            zero_ratio = (activation == 0.).sum().item() / activation.numel()
